...
;				 { loc_data - named variable to receive file data
;					 this can be passed in for looping runs
;
; KEYWORD PARAMETERS:
;
...
;			Reconfigured to return data structure, so as not to re-read file each time
;			loc_data input/output added	MDS2Apr03
;			Added write crossing data to file for when set, MDS 11Jun03.
;==============================================================================

PRO filtcellarc, file, filt_out, cells, loc_data,$
		time0 = time0, $
		time1 = time1, $
		beasts = beasts, $
		scale = scale, $
...
IF size(loc_data,/type) NE 8 THEN BEGIN  ;this assumes you want the same data from the same file
...
IF keyword_set(crossing_data) THEN mdswrite_crossings, filt_out, cells, labl + 'crossings.txt', /crossing_data
