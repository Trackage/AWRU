...
pro write_crossings, argos_data, cell_data, $
...
openw, wlun,'crossing_times.txt',width=150,/get_lun

printf, wlun, "  From " + dt_tm_fromjs(start_time,format='d$/n$/Y$ h$:m$') + $
...
for k=1,n_elements(cell_data.cross_times)-1 do begin
    ...
    printf, wlun, cell_data.cross_ptt(k), ' ', $
        dt_tm_fromjs(cell_data.cross_times(k-1), format='d$/n$/Y$ h$:m$'), ' ', $   ;-- entry time
        dt_tm_fromjs(cell_data.cross_times(k), format='d$/n$/Y$ h$:m$'), '    ', $  ;-- exit time
