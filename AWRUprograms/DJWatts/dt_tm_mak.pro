...
;-------------------------------------------------------------
	...
	st(*).sec_after_midnight = rem		; Save value.
	...
	st(*).jd = jd0+days			; Save value.
	st(*).year=y & st(*).month=m & st(*).day=d	; Save values.
	if n_elements(ddec) eq 0 then ddec=0	; Default for doy is integer.
	tmp = ymd2dn(y,m,d)			; Day of year.
	st(*).day_of_year = tmp + rem/86400d0	; Save value.
	...
	st(*).weekday = nwd			; Save value.

	sechms, rem, h, m, s, hh, mm, ss	; Find Hr, Min, Sec.
	st(*).hour=h & st(*).minute=m & st(*).second=s	; Save values.
	...
	for i = 0, num-1 do begin
	  ...
	  tmp = stress(tmp, 'R', 0, 'sam$',sam(i)) ; Seconds after midnight.
