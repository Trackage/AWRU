...
;              8 bit data. (rsa)
; 01 Nov 2001: Type cast the assignment values for variables ixbin
;              and iybin. (ie. originally int = float) updated to
;              (int = fix(float))
;===================================================================
...
forrd,3,xh1,xh2,xh3,xh4,data_read
;--------------------------------------------------------
...
for ix=0,num_points - 1  do begin
   for iy=0,ndatablocks - 1  do begin
      ixbin=fix((float(temp_lon(0,ix,iy))/100.-xlonmn)*xibin)
      iybin=fix((float(temp_lat(0,ix,iy))/100.-xlatmn)*yibin)
