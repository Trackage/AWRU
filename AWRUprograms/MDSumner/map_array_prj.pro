;============================================================================
PRO MAP_ARRAY_PRJ, ARRAY, LONS, LATS, PAL = PAL, TITLE = TITLE, FRONTS = FRONTS
;==============================================================================
; NAME:
;      	MAP_ARRAY.PRO
; PURPOSE:
...
map_set, P0lat, P0lon, $
	limit = [min(lats), min(lons), max(lats), max(lons)], $
	 /cylindrical, xmargin = [0, 18], ymargin = 0, color = !d.n_colors - 1, title = title

IF stru_flag EQ 0 THEN maparray = rotate(array, 7)    ;flip the array
;IF stru_flag EQ 0 AND n_elements(array2) NE 0 THEN maparray2 = rotate(array2, 7)    ;flip the array
...
;	remap2 = map_image(maparray2, x0, y0, xsize, ysize, compress=1, lonmin = min(lons), $
	;	lonmax = max(lons), latmin = min(lats), latmax = max(lats))
...
map_continents, color = col, /hires, /fill_continents
map_grid, lonlab = min(lats) + .5, latlab = max(lons) -2, /label, color = 0, $;!d.n_colors - 1, $
	charsize = 1.5
