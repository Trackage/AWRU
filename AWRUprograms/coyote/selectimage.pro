...
; This event handler simply flips the !Order system variable
; and re-displays the image in the window.

Widget_Control, event.top, Get_UValue=info, /No_Copy

   ; Flip the !Order system variable.

!Order = 1-!Order

   ; Get the image and re-display it.

image = *(*(info.storagePtr)).image
...
filterID = Widget_Button(buttonBase, Value='Image Type', Menu=2, Event_Pro='SelectImage_SetFilter')
...
button = Widget_Button(buttonBase, Value='Accept', Event_Pro='SelectImage_Action')
