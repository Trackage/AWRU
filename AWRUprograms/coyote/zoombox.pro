...
Widget_Control, event.top, Set_UValue=info, /No_Copy
END ; of ZOOMBOX_FACTOR ***********************************************************



PRO ZOOMBOX_BUTTON_DOWN, event
   ...
   ; the static corners of the zoom box, (2) changes the event handler for
   ; the draw widget to ZOOMBOX_DRAWBOX and turns on MOTION events,
...
CASE thisEvent OF
   ...
   'DOWN': BEGIN
      ...
      Widget_Control, event.id, Event_Pro='ZOOMBOX_DRAWBOX', $
...
END ; of ZOOMBOX_BUTTON_DOWN *****************************************************



PRO ZOOMBOX_DRAWBOX, event
...
IF thisEvent EQ 'UP' THEN BEGIN
      ...
      ; OFF, set the draw widget's event handler back to ZOOMBOX_BUTTON_DOWN, and
      ; draw the "zoomed" plot.

      ; Turn motion events off and redirect the events to ZOOMBOX_BUTTON_DOWN.

    Widget_Control, event.id, Draw_Motion_Events=0, $
       Event_Pro='ZOOMBOX_BUTTON_DOWN'
   ...
   info.theModel->SetProperty, Hide=0
      ...
      ; Redisplay the image one last time.
...
Widget_Control, event.top, Set_UValue=info, /No_Copy
END ; of ZOOMBOX_DRAWBOX ******************************************************************



PRO ZOOMBOX, image, Group_Leader=group, Interpolation=interp
...
drawID = Widget_Draw(drawbase, XSize=xsize, YSize=ysize, Retain=0, $
   Button_Events=1, Event_Pro='ZOOMBOX_BUTTON_DOWN', Graphics_Level=2, $
   Expose_Events=1, Renderer=1)
