...
IF N_Elements(vectorMin) EQ 0 THEN BEGIN
   vectorMin = Float( Min(vector, NAN=Keyword_Set(nan)) )
ENDIF ELSE BEGIN
   vectorMin = Float( vectorMin )
ENDELSE

IF N_Elements(vectorMax) EQ 0 THEN BEGIN
   vectorMax = Float( Max(vector, NAN=Keyword_Set(nan)) )
ENDIF ELSE BEGIN
   vectorMax = Float( vectorMax )
ENDELSE
