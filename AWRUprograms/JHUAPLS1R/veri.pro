...
;-------------------------------------------------------------
        ...
        pro veri, x, color=color, linestyle=linestyle, $ 
	...
	common js_com, jsoff
        ...
        if keyword_set(hlp) then begin 
          print,' Interactive vertical line on screen or plot.' 
          print,' veri, [x]' 
	  ...
	  print,'     each call to veri (stays in same position).'
	  ...
	  print,'   BUTTON=b   Returned button code: 1=left, 2=middle, 4=right.'
          ...
          print,'   X may be set to starting position in entry.' 
          return 
        endif 
	...
	xdv = tmp(0)<xxdv(1)  & ydv = tmp(1)<yydv(1)
        ...
        if not keyword_set(xmode) then t=tvrd(xdv,0,1,!d.y_size) ; 1st col.
	...
	;---------  Set up status widget  -------------
        if stat then begin
          if not widget_info(top,/valid_id) then begin
            top = widget_base(/column,title='Interactive Vertical Line')
	...
	xcl = -2  & ycl = -2
	  ...
	  xcl = xdv  & ycl = ydv
          if not keyword_set(xmode) then begin
            tv, t, xl, 0                          ; Replace last column. 
            t = tvrd(xdv,0,1,!d.y_size)           ; Read new column.
          endif else begin
