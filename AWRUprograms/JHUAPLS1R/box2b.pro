...
;-------------------------------------------------------------
	pro box2b, x10, x20, y10, y20, status=stat, help=hlp, $
	  exitcode=exit, menu=menu, nomenu=nomenu, yrev=yrev
	...
	noerase = 1			; Don't erase old box first time.
        ...
        if n_elements(menu) eq 0 then menu=['OK','Abort','List Box']
	...
	if min([x10,x20,y10,y20]) ge 0 then begin
	  ...
	  if keyword_set(stat) then begin	; Update status.
	    ...
	    widget_control,nid(1),set_val='Y1  Y2  DY  =  '+strtrim(y1out,2)+$
		'   '+strtrim(y2out,2)+'   '+strtrim(y2-y1+1,2)
...
loop:
	...
	;-------  Exit box routine  ------------
	  ...
	  if exit eq 2 then begin
            x1s = strtrim(x1,2)
	    nx1s = strtrim(float(x1)/!d.x_size,2)
            x2s = strtrim(x2,2)
	    nx2s = strtrim(float(x2)/!d.x_size,2)
            dxs = strtrim(x2-x1+1,2)
            y1s = strtrim(y1,2)
	    ny1s = strtrim(float(y1)/!d.y_size,2)
            y2s = strtrim(y2,2)
	    ny2s = strtrim(float(y2)/!d.y_size,2)
	    dys = strtrim(y2-y1+1,2)
            cxs = strtrim((x1+x2)/2,2)
            cys = strtrim((y1+y2)/2,2)
	    if keyword_set(yrev) then begin
	      y2s = strtrim(!d.y_size-y1-1,2)
	      y1s = strtrim(!d.y_size-y2-1,2)
              cys = strtrim(!d.y_size-(y1+y2)/2-1,2)
	    endif
	    print,' '
	    print,' Box position:'
	    print,'   X1, X2, DX, CX = '+x1s+',  '+x2s+',  '+dxs+',  '+cxs
	    print,'   Y1, Y2, DY, CY = '+y1s+',  '+y2s+',  '+dys+',  '+cys
	    print,' Screen position of box:'
	    print,'   ,/dev, pos=['+x1s+','+y1s+','+x2s+','+y2s+']'
	    print,'   ,/norm, pos=['+nx1s+','+ny1s+','+nx2s+','+ny2s+']'
