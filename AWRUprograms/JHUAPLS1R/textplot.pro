...
;-------------------------------------------------------------
	pro textplot, x, y, txt, charsize=csz, orient=ang, alignment=jst, $
	  ...
	  image=img, xcrop=xcrp, ycrop=ycrp
...
hlp:	  print,' Plot text with 2-d alignment.  Also return bounding box.'
	  ...
	  print,'   BOLD=bld Text bold value (may be 1-d array).'
	  ...
	  print,'   /DEVICE, /NORMAL, /DATA coordinate system to use.  If not'
	  print,'     given will default to normal if x & y between 0 & 1, or'
	  print,'     device if x & y in range, or else data.'
	...
	;----------------------------------------------
	;  Determine coordinate system from given
	;  reference point (x,y).
	;----------------------------------------------
	cflag = 0					  ; None.
	...
	if cflag eq 0 then begin			  ; Still none.
	  cflag = 3					  ; Default to /DATA.
	  if (x ge 0) and (x lt !d.x_size) and $	  ; or /DEV if works.
	     (y ge 0) and (y lt !d.y_size) then cflag=1
	  if (x ge 0.) and (x lt 1.) and $		  ; or /NORM if works.
	     (y ge 0.) and (y lt 1.) then cflag=2
	endif
	;----------------------------------------------
...
3:	begin
	...
	;----------------------------------------------
	xoff0 = dx*fx		; Non-rotated offset.
	yoff0 = dy*fy
