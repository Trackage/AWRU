...
;=================================================================
	...
	pro xcurve, xa, ya, device=dev, data=data,normal=norm, $
	  mag=mag0, size=msize0, step_min=step, help=hlp
	...
	while !mouse.button ne 4 do begin
	  ...
	  if !mouse.button eq 1 then begin
	    ...
	    tm = !mouse.time
	    dtm = abs(ltm-tm)
	    if dtm gt 100 then begin
	      ltm = tm
	      ;-------  Avoid repeated points  --------
	      dx = x-xa((n-1)>0)			; Find dist from last.
	      dy = y-ya((n-1)>0)
	  ...
	  ;-------  Middle button: Drop point.  ---------
		...
		data=data,dev=dev,norm=norm
              xplot, xa((n-1)>1),xa(n),ya((n-1)>1),ya(n), $	; Deleted line.
		data=data,dev=dev,norm=norm
