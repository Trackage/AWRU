...
;-------------------------------------------------------------
	...
	pro sphlng, lng, rad, lat1, lat2, help=hlp, $
	...
	if (n_params(0) lt 2) or keyword_set(hlp) then begin
	  print,' Draw meridians of longitude on a sphere.
	  print,' sphlng, lng, rad, [lat1, lat2]'
	  print,'   lng = longitude of meridian.              in'
	  print,'   rad = radius of sphere.                   in'
	  print,'   lat1, lat2 = latitude range of meridian.  in'
	  print,' Keywords:'
	  ...
	  return
        ...
        ;          Set default values          ;
        ;--------------------------------------;
	if n_elements(lat1) eq 0 then lat1 = -90.
	if n_elements(lat2) eq 0 then lat2 = 90.
	...
	if n_elements(thick) eq 0 then thick = !p.thick
        ...
        ;--------------------------------------------------;
	;---  Set up plot points in spherical polar coordinates  ----
	lata = makex(lat1<lat2, lat1>lat2, inc0)
	n = n_elements(lata)
	lnga = maken(lng, lng, n)
