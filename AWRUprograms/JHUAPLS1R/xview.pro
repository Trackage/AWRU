...
'CUSTOM': begin
	...
	mx = smax(0)
	my = smax(1)
	size_img, img, true=tr, nx=nx,ny=ny
	...
	if mag ne 1 then begin
	  img = img_resize(img,mag)
	  size_img, img, nx=nx,ny=ny
	endif
	sx = nx<mx
	sy = ny<my
	...
	if (sx ne nx) or (sy ne ny) or (sx lt 50) then begin
	  swindow, xs=nx, ys=ny, x_scr=sx+4, y_scr=sy+4, $
	    title=tt, index=tmp
	  win = tmp(0)
	endif else begin
...
;========================================================
	...
	if name eq 'HOT' then begin
	  ...
	  allowed = ['gif','srf','jpg','xwd','tif','bmp','xbm','pict']
	  allowed2 = ['GIF','SRF','JPEG','XWD','TIFF','BMP','XBM','PICT']
	  w = (where(btyp eq allowed, cnt))(0)
