...
;-
;-------------------------------------------------------------
	pro gentile, init=init, x, y, color=clr, thick=thk, linestyle=sty, $
	  fill=fclr, notes=notes, help=hlp
 
	common gentile_com, nx, ny
	...
	if keyword_set(notes) then begin
	  ...
	  print,'     gentile,init=[100,100]'
	  print,'     for i=0,9 do gentile,x*r(i)+xo(i),y*r(i)+yo(i)'
	...
	if n_elements(nx) eq 0 then begin
	  print,' Error in gentile: you must first initialize tile'
	  print,'   by gentile,init=[nx,ny]'
	  return
	endif
