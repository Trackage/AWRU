...
;--------------------------------------------------------------------
	...
	if uval eq 'TIMER' then begin
	  ...
	  widget_control, s.b_timer, timer=s.sec	; Yes, next timer.
	  return
	endif
 
	if uval eq 'OPEN' then begin
	  ;-----  Drop old window if any  -----
	  wdelete				; Drop any existing window.
...
infile:	  xtxtin, file, title='Enter name of res movie file:',$
	    xoff=50,yoff=100, def=last_file
...
;	  aa = assoc(lun, bytarr(xsize,ysize), offset)	; Set up assoc.
	  ...
	  ;----  Set up window  ----
	  window,xs=xsize,ys=ysize			; Make a window.
...
;	  widget_control, ev.top, set_uval=s	; Save updated info.
	...
	if uval eq 'OPT2' then begin
	  xtxtin,new,title='Change frame step size:',def=strtrim(s.step,2)
	...
	if uval eq 'OPT3' then begin
	  xtxtin,new,title='Change frame delay seconds:',def=strtrim(s.sec,2)
	  if new eq '' then return
	  s.sec = new+0.
	  widget_control, ev.top, set_uval=s
	  return
...
;--------------------------------------------------------------------
	...
	;--------  Pack info  ------------------------
	info = { $
	  time_flag:0, b_time:b_time, id_time:id_time, $
	  id_slid:id_slid, step:1, win:0, $
	  b_timer:b_timer, mode:0, id_frame:id_frame, $
	  sec:0.01, lun:0L, index:0, b_buttons:b_buttons, num:100, $
	  set:[1,1,1], id_vs1:id_vs1, id_vs2:id_vs2, id_vl1:id_vl1, $
	  id_vl2:id_vl2, id_vslid:id_vslid, vmode:1, id_lut:id_lut, $
	  cmd:cmd2, order:order, c_base:c_base }
