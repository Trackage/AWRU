...
;-
;-------------------------------------------------------------
	function img_smooth, img, sm, width=width, error=err, $
	  zero=zero, help=hlp
 
	if (n_params(0) lt 1) or keyword_set(hlp) then begin
	  print,' Smnooth a 2-D or 3-D image array.'
	  print,' out = img_smooth(in,sm)'
	...
	if (ndim lt 2) or (ndim gt 3) then begin
	  err = 1
	  print,' Error in img_smooth: given array must 2-D or 3-D.'
	  return, img
	endif
	...
	if typ eq 0 then begin
	  err = 2
	  print,' Error in img_smooth: given array must have a dimension of 3.'
	  return, img
	endif
